...
describe('Test suite for the /users routes on our api', () => {
    ...
    test('It should create a new user in the db', async () => {
        const response = await request(app).post('/users').send({ name: 'Rachail Cool', email: 'Rachail@chill.com', password: '12345'})

        expect(response.statusCode).toBe(200)
        expect(response.body.user.name).toEqual('Rachail Cool')
        expect(response.body.user.email).toEqual('Rachail@chill.com')
